...
KELVIN_RMIN = 203.15 * 100.0
KELVIN_RMAX = 307.15 * 100.0
...
def humanRange(data):
	d = cv2.inRange(data, KELVIN_RMIN, KELVIN_RMAX)
	return cv2.cvtColor(np.uint8(d), cv2.COLOR_GRAY2RGB)
		...
		try:
			...
			try:
				...
				while True:
					...
					data2 = cv2.resize(data2[:, :], (640, 480))
					data = humanRange(data)
					minVal, maxVal, minLoc, maxLoc = cv2.minMaxLoc(data2)
					...
					data2[-1][-1] = max_c
					#print minVal, maxVal
					img = raw_to_8bit(data2)
					img2 = cv2.LUT(img, colour_map)
					fgmask = fgbg.apply(img)
					fgmask = cv2.cvtColor(np.uint8(fgmask), cv2.COLOR_GRAY2RGB)
					#last = cv2.addWeighted(img, 1, fgmask, 1, 0.0)

					_, img = cv2.threshold(cv2.cvtColor(np.uint8(img), cv2.COLOR_RGB2GRAY), 205, 256, cv2.THRESH_BINARY)
					_, contours2, hierarchy2 = cv2.findContours(np.uint8(img), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
					...
					objects = "objects: {}".format(len(contours2))
					font = cv2.FONT_HERSHEY_SIMPLEX
					time_str = "{:s} ({:.2f}, {:.2f}) {:s}".format(
						timestr, ktoc(minVal), ktoc(maxVal), objects)
					cv2.putText(img2, time_str, (10, 26),
								font, 0.70, (255, 255, 215), 1, 2)

					cv2.imshow('test', img2)
					cv2.imshow('raw', img)
					cv2.imshow('raw2', data)
					cv2.waitKey(1)
